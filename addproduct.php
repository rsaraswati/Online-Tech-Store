...
<body id="addadminpage">
    ...
    <form method="POST" action="addproduct.php" name="addproduct" id="productform">
        ...
        <div class="labelsection"><span>Mark as Deleted (Y/N)</span></div><div class="inputsection">
            <select name="ddldelete" class="addinput prodelete">
                <option value="Y"   <?php if ($deletedflag === 'Y') echo ' selected="selected"' ?>>Yes</option>
                <option value="N"   <?php if ($deletedflag === 'N') echo ' selected="selected"' ?>>No</option>                
            </select>
        </div><br/><br/><br/>
...
function InsertProduct()
{
    ...
    try{    
        ...
        $query->bindParam(':deletedflag', $_POST["ddldelete"], PDO::PARAM_STR);
        $query->execute();        
    }catch(PDOException $ex){
